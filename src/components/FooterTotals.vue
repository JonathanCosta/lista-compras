...
const { totalPrevisto, totalNoCarrinho } = useTotals()
</script>

<template>
  <footer
    class="sticky bottom-0 z-50 border-t shadow-lg"
  ...
  >
    <div class="max-w-lg mx-auto px-4 py-3 flex justify-between items-center">
      <div class="text-sm">
        ...
        >{{ formatCurrency(totalPrevisto) }}</span>
      </div>
      <div class="text-sm flex items-center gap-2">
        ...
        />
        <span class="text-gray-500">No carrinho: </span>
        <span
          data-testid="total-carrinho"
          class="font-bold"
          style="color: var(--color-brand);"
        >{{ formatCurrency(totalNoCarrinho) }}</span>
      </div>
    </div>
  </footer>
</template>
